...
export interface UserAuthDelegate<ExtArgs extends runtime.Types.Extensions.InternalArgs = runtime.Types.Extensions.DefaultArgs, GlobalOmitOptions = {}> {
   ...
   * })
   * 
   * // Create many UserAuths and only return the `userId`
   * const userAuthWithUserIdOnly = await prisma.userAuth.createManyAndReturn({
   *   select: { userId: true },
   *   data: [
   *     // ... provide data here
   *   ]
   ...
   * })
   * 
   * // Update zero or more UserAuths and only return the `userId`
   * const userAuthWithUserIdOnly = await prisma.userAuth.updateManyAndReturn({
   *   select: { userId: true },
   *   where: {
   *     // ... provide filter here
   *   },
